...
const dummySurveys = [
    ...
    { id: 1, title: '설문조사 16', subtitle: '설문조사 16 설명', tags: ['교육', '기타'], progress: 50, endDate: '2024-10-20', participants: 110, job: ['학생', '프리랜서'], gender: '남성', age: ['10대', '20대'], region: ['경기', '서울'] },
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        if (filters.age && filters.age.includes('')&& filters.age.length) {
          ...
          filteredSurveys = filteredSurveys.filter(survey => 
            survey.age.some(ageRange => filters.age && filters.age.includes(ageRange))
          );
        }
        // 페이지 처리
